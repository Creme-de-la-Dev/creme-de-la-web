...
function updatecurrentRoom(roomparameter) {
	...
	if (roomparameter != "6") {
		...
		var roomState = currRoomStates[currentroomstatetoset2].roomstate;
		// Safety check: make sure the room state and image exist
		if (roomImages[currentRoomID] && roomImages[currentRoomID][roomState]) {
			roomdiv.attr("src", roomImages[currentRoomID][roomState].src);
		} else {
			debuglog("ERROR: Missing image for room " + roomparameter + " state " + roomState);
			console.error("Missing image for room " + roomparameter + " state " + roomState + " (roomID: " + currentRoomID + ")");
			// Fallback to state 0
			if (roomImages[currentRoomID] && roomImages[currentRoomID][0]) {
				roomdiv.attr("src", roomImages[currentRoomID][0].src);
			}
		}
...
function mainmenu() {
	...
	const adminMode = urlParams.get('naul') === 'true';
	...
	if (localStorage["fnaf-js-savegame.night"] != "1" || adminMode) {
		...
		if (parseInt(localStorage["fnaf-js-savegame.night"]) < 5 && !adminMode) {
			...
			if (localStorage["fnaf-js-savegame.night"] == "7" || adminMode) document.getElementById("customnight").style.display = "block";
		}
	};
